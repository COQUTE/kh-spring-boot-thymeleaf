...
UPDATE "MEMBER" SET
MEMBER_PW = '$2a$10$Zmzgl0a93LYytP.7MP2E5eYI/w7eLcJb02TeDvPhvV9UNviACyHk.'
WHERE MEMBER_NO = 1;
...
FROM "MEMBER"
WHERE MEMBER_EMAIL = 'user01@kh.or.kr' AND MEMBER_DEL_FL = 'N';
...
INSERT INTO "BOARD_LIKE" VALUES(1, 2000); -- 1번 회원이 1994번 게시글에 좋아요를 누름
...
CASE
	WHEN SYSDATE - BOARD_WRITE_DATE < 1 / 24 / 60 THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60 * 60) || '초 전'
	WHEN SYSDATE - BOARD_WRITE_DATE < 1 / 24 THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24 * 60) || '분 전'
	WHEN SYSDATE - BOARD_WRITE_DATE < 1 THEN FLOOR((SYSDATE - BOARD_WRITE_DATE) * 24) || '시간 전'
...
FROM "BOARD_IMG"
WHERE BOARD_NO = 2000
AND IMG_ORDER = 0) AS THUMBAIL,

(SELECT COUNT(*)
FROM "BOARD_LIKE"
WHERE BOARD_NO = 2000
AND MEMBER_NO = 1) AS LIKE_CHECK
...
-- LEVEL : Oracle에서 제공하는 계층형 데이터 조회할 때 몇 "레벨"에 있는지 알려주는 가상의 컬럼
SELECT LEVEL, C.*
...
SELECT CHATTING_ROOM_NO
	...
	,NVL2((SELECT OPEN_MEMBER FROM CHATTING_ROOM R2
		...
		R.PARTICIPANT, -- NULL이 아닌 경우 (로그인한 사람 오픈 멤버일 때는 참가자 타겟번호)
