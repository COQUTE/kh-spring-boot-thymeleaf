...
function getTotalCount() { // 함수 정의
  ...
  // fetch() API 코드 작성
  fetch("/ajax/totalCount") // 서버 "/ajax/totalCount"로 GET 요청
...
changeComplete.addEventListener("click", () => {
  ...
  // SQL 수행에 필요한 두 값을 JS Object로 묶음
  const obj = {
  ...
  .then(result => {
    ...
    if(result > 0) { // 완료여부 변경 성공
      ...
      // selectTodo();
      // 상세 조회에서 Y/N 만 바꾸기
      popupComplete.innerText = obj.complete; 
      
      // getCompleteCount();
      // 완료된 Todo 개수 +-1

      const count = Number(completeCount.innerText);
      if(obj.complete === 'Y') completeCount.innerText = count + 1;
