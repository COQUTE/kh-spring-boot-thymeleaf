...
// 2) 이메일이 입력될 때(input)마다 유효성 검사 수행
memberEmail.addEventListener("input", (e) => {
  ...
  // 영어, 숫자, 특수문자(. _ % +)를 허용
  // 이메일 형식으로 작성
  // 도메인 부분은 최소 2개의 문자로 끝나야함 (.com, .net, .org, .kr)
  // 이메일 정규식 검사 정규표현식
...
sendAuthKeyBtn.addEventListener("click", () => {
  ...
  // 새로 발급받은 인증번호 확인 전까진 checkObj.authKey는 false
  checkObj.authKey = false;
  // 인증번호 발급 관련된 메시지 지우기
  authKeyMessage.innerText = "";
...
checkAuthKeyBtn.addEventListener("click", () => {
  ...
  if(authKey.value.length !== 6) { // 인증번호가 제대로 입력 안된 경우 (길이가 6미만 또는 7이상)
...
memberPw.addEventListener("input", e => {
  ...
  // 유효한 경우
  pwMessage.innerText = "사용가능한 비밀번호입니다.";
  pwMessage.classList.remove('error');
  pwMessage.classList.add('confirm');
